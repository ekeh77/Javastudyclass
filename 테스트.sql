...
(
    ...
    SELECT T1.SID,T1.SNAME,T1.STEL,T1.SADDR,T1.SGENDER,T1.SAGE,T1.SUBID,T1.SCORE,T2.SUBID,T2.SCORE,T3.SUBID,T3.SCORE,
    T4.SUBID,T4.SCORE,T5.SUBID,T5.SCORE,T6.SUBID,T6.SCORE
    FROM
    (SELECT T1.SID,T1.SNAME,T1.STEL,T1.SADDR,T1.SGENDER,T1.SAGE, T2.SUBID,T2.SCORE
    FROM STUDENTS T1, (SELECT * FROM SCORES WHERE SUBID = 'S001') T2
    WHERE T1.SID = T2.SID(+)
    AND T1.SGENDER = 'F'
    AND (T1.SAGE >= 20 AND T1.SAGE < 30))T1,
    (SELECT T1.SID,T2.SUBID,T2.SCORE
    FROM STUDENTS T1, (SELECT * FROM SCORES WHERE SUBID = 'S002') T2
    WHERE T1.SID = T2.SID(+)
    AND T1.SGENDER = 'F'
    AND (T1.SAGE >= 20 AND T1.SAGE < 30))T2,
    (SELECT T1.SID,T2.SUBID,T2.SCORE
    FROM STUDENTS T1, (SELECT * FROM SCORES WHERE SUBID = 'S003') T2
    WHERE T1.SID = T2.SID(+)
    AND T1.SGENDER = 'F'
    AND (T1.SAGE >= 20 AND T1.SAGE < 30))T3,
    (SELECT T1.SID,T2.SUBID,T2.SCORE
    FROM STUDENTS T1, (SELECT * FROM SCORES WHERE SUBID = 'S004') T2
    WHERE T1.SID = T2.SID(+)
    AND T1.SGENDER = 'F'
    AND (T1.SAGE >= 20 AND T1.SAGE < 30))T4,
    (SELECT T1.SID,T2.SUBID,T2.SCORE
    FROM STUDENTS T1, (SELECT * FROM SCORES WHERE SUBID = 'S005') T2
    WHERE T1.SID = T2.SID(+)
    AND T1.SGENDER = 'F'
    AND (T1.SAGE >= 20 AND T1.SAGE < 30))T5,
    (SELECT T1.SID,T2.SUBID,T2.SCORE
    FROM STUDENTS T1, (SELECT * FROM SCORES WHERE SUBID = 'S006') T2
    WHERE T1.SID = T2.SID(+)
    AND T1.SGENDER = 'F'
    AND (T1.SAGE >= 20 AND T1.SAGE < 30))T6
    WHERE T1.SID = T2.SID
    AND T1.SID = T3.SID
    AND T1.SID = T4.SID
    AND T1.SID = T5.SID
    ...
    SELECT T2.ENAME, SUM(T3.SCORE) AS 총점, COUNT(*) --, AVG(T3.SCORE) --/ ,SUM(T3.SCORE) /COUNT( ?? ) AS 평균
    FROM STUDENTS T1, EMAMS T2, SCORES T3, STUDENTS T4
    WHERE T1.SID = T3.SID(+)
    AND T2.SUBID = T3.SUBID(+)
    AND T3.SID(+) = T4.SID
    AND T1.SGENDER = 'F' AND T1.SAGE >=20 AND T1.SAGE < 30
    GROUP BY T2.ENAME
    ORDER BY T2.ENAME ASC
    ;
